...
class HiddenLayer(object):
    ...
    def hidden_output(self):
        print 'type(self.output):', type(self.output) 
        return T.mul(self.output,1)
...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations,use_bias=True):
        ...
        self.predict_lr = self.layers[-1].predict_lr
        self.predict_p_lr = self.layers[-1].predict_p_lr
    ...
    def predict(self, new_data):
        ...
        y_pred = T.argmax(p_y_given_x, axis=1) # argmax 
        return y_pred
        
    def predict1(self, new_data):
        next_layer_input = new_data
        for i,layer in enumerate(self.layers):
            if i<len(self.layers)-1:
                next_layer_input = self.activations[i](T.dot(next_layer_input,layer.W) + layer.b)
    ...
    def predict_p(self, new_data):
        ...
        for i,layer in enumerate(self.layers):
            if i<len(self.layers)-1:
                ...
                p_y_given_x = T.nnet.softmax(T.dot(next_layer_input, layer.W) + layer.b)
        return p_y_given_x # p_y_given_x 
        
    def predict_p1(self, new_data):
        next_layer_input = new_data
        for i,layer in enumerate(self.layers):
            if i<len(self.layers)-1:
                next_layer_input = self.activations[i](T.dot(next_layer_input,layer.W) + layer.b)
...
class LogisticRegression(object):
    ...
    def predict_p_lr(self):
        return T.mul(self.p_y_given_x,1)
